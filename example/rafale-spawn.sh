...
start() {
  ...
  if [ -f $PID_FILE ]; then
    echo "PID file exists, if YOU ARE SURE cgi is not running remove [$PID_FILE]"; exit
  fi
  echo spawn-fcgi -a $ADDRESS -p $PORT -u $USER -g $GROUP -P "$PID_FILE" -f "$CGI_FILE";
}

stop() {
  echo "Stopping spawn-fcgi"
  if [ ! -f "$PID_FILE" ]; then
    echo "PID file not found, if YOU ARE SURE cgi is running, try force-stop"; exit
  fi
  kill $(cat "$PID_FILE")
  rm "$PID_FILE" 
}
